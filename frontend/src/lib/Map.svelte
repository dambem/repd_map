<script>
    ...
    function wireInteractions() {
        ...
        map.on('mousemove', (e) => {
            const queryLayers = [LAYER.GRAVES, LAYER.PROJECTS, LAYER.AUTHORITIES].filter((id) =>
                map.getLayer(id),
            );
            ...
            const point = features.find(
                (f) => f.layer.id === LAYER.PROJECTS || f.layer.id === LAYER.GRAVES,
            );
        ...
        for (const layerId of [LAYER.PROJECTS, LAYER.GRAVES]) {
        ...
        map.on('click', LAYER.AUTHORITIES, (e) => {
            ...
            const markerLayers = [LAYER.PROJECTS, LAYER.GRAVES].filter((id) => map.getLayer(id));
    ...
    function applyControls() {
        ...
        map.setPaintProperty(
            LAYER.PROJECTS,
            'circle-color',
            colorMode === 'nimby' ? nimbyColorExpression(nimbyIndex) : typeColorExpression(),
        );
